...
class AgentRunner:
    ...
    async def _build_system_prompt(
        ...
        section_parts["platform_knowledge"] = (
            ...
            "Take the user anywhere via `navigate_to(path)` (explicit request) "
            "or `[[navigate:/path]]` chip (suggestion).\n\n"
            ...
            "progress. Live job card visible at `/jobs`. For recurring "
            "tasks ('remind me every morning at 8') → `cron`.\n\n"
            "### Media\n"
            ...
            "## Decision rules — when user says X, you do Y\n\n"
            "| User says | You do |\n"
            "| --- | --- |\n"
            "| 'remember <fact>' / 'I'm working on <project>' | `memory_store` (silent or one-line confirm) |\n"
            "| 'what do you know about <X>' | `memory_search` |\n"
            "| 'show me my memories' / 'what have you saved' | `navigate_to('/brain')` |\n"
            "| 'make me a <tool/app>' / 'I need a <thing>' | app_builder skill |\n"
            "| 'search the web' / 'find <X> for me' / 'book <X>' | `browser` (suggest `[[navigate:/browser]]` if they should watch) |\n"
            "| 'schedule <X>' / 'remind me at <Y>' | `cron` |\n"
            "| 'make this a PDF/doc/spreadsheet/deck' | `generate_pdf` / `generate_docx` / `generate_xlsx` / `generate_pptx` |\n"
            "| 'play <song/movie/show>' | `play_media` |\n"
            "| 'open settings' / 'wire up whatsapp' / 'change my phone' | `navigate_to('/agent/settings')` |\n"
            "| 'change your name' / 'change your personality' | `navigate_to('/agent/soul')` |\n"
            "| 'what tools do you have' | `navigate_to('/agent/tools')` |\n"
            "| 'yesterday's chat' / 'what did we talk about Monday' | `recall_day` |\n"
            "| 'where's my account' / 'change password' / 'billing' | `navigate_to('/account')` |\n"
            "| 'show me the dashboard' / 'metrics' | `navigate_to('/dashboard')` |\n"
            "| 'build me an app' (then they want to see it) | app_builder, then `[[open_app:<slug>]]` |\n\n"
