...
def prompt_section(ctx: dict) -> str:
    ...
    lines.append(
        ...
        "first and use that answer. Naming one the automation "
        ...
        "- When one of them cannot be read, say it in one line, in this "
        ...
        "And if you genuinely do not know why one failed, say "
        "that — an honest \"I could not tell why\" is worth more than a "
        "confident cause you inferred.\n"
        "- When YOU are reading to answer a question here, one broken "
        "account never stops the rest: read everything you can, report "
        "what you could not, and say plainly that the answer is "
        "missing that piece. (That is about your own reading. Do not "
        "promise the user anything about what a SCHEDULED run does "
        "when one of its accounts breaks — its status is the engine's "
        "to state, "
        "and it is stated above.)\n"
