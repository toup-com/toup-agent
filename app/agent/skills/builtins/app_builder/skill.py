...
class AppBuilderSkill(Skill):
    ...
    def get_system_prompt_section(self) -> Optional[str]:
        return (
            ...
            "- **Layer 2** — Personalization the user does through conversation with you inside the app.\n\n"
            "When the user triggers Layer 2 (clicks 'Customize this app' or similar from the in-app chat panel):\n\n"
            "**Step 1: Understand the App**\n"
            "- Use `app_{slug}__read_file` to read the app's key files: App.tsx, screen components, database schema, navigation structure.\n"
            "- Build a mental model of: all screens, features, database tables, navigation flow, color theme, and current functionality.\n\n"
            "**Step 2: Ask 10+ Targeted Customization Questions**\n"
            "Ask at least 10 questions about how to personalize the app for the user's specific needs.\n"
            "Questions MUST be specific to what the app actually does — not generic.\n"
            "Every question MUST have [[option]] buttons. NO exceptions.\n\n"
            "Categories to cover:\n"
            "- **Content**: What specific data/content should be pre-loaded? What categories/topics matter most?\n"
            "- **Workflow**: How does the user prefer to use the app? What's their typical session like?\n"
            "- **Goals**: What are they trying to achieve? Specific targets, deadlines, milestones?\n"
            "- **Preferences**: Color adjustments, layout changes, font sizes, dark/light mode?\n"
            "- **Features**: Which features should be prominent? Which should be hidden? Any new features?\n"
            "- **Data**: Import existing data? Pre-fill with relevant content? Default settings?\n"
            "- **Notifications**: Reminders, streaks, daily goals, progress alerts?\n"
            "- **Difficulty/Level**: Beginner vs advanced content? Pacing preferences?\n\n"
            "Example for an IELTS prep app:\n"
            "```\n"
            "Let me customize this app for you! I've read through the entire app structure. Here are some questions:\n\n"
            "1. **What's your current IELTS band score?**\n"
            "[[4.0-4.5]] [[5.0-5.5]] [[6.0-6.5]] [[7.0+]] [[Never taken]]\n\n"
            "2. **Which section needs the most work?**\n"
            "[[Reading]] [[Writing]] [[Listening]] [[Speaking]] [[All equally]]\n\n"
            "3. **When is your test date?**\n"
            "[[In 2 weeks]] [[In 1 month]] [[In 3 months]] [[No date yet]]\n"
            "```\n\n"
            "**Step 3: Apply Changes**\n"
            "Based on answers, use `app_{slug}__write_file` and `app_{slug}__query_db` to:\n"
            "- Modify screen content, labels, and copy to match user's context\n"
            "- Adjust difficulty levels, content ordering, and feature visibility\n"
            "- Pre-populate database with relevant data (study plans, schedules, goals)\n"
            "- Update color theme, layout, and navigation if requested\n"
            "- Add or modify features based on preferences\n\n"
            "Show progress as you make changes. After each file edit, confirm what was changed.\n"
