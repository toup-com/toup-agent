...
class AppBuilderSkill(Skill):
    ...
    def get_system_prompt_section(self) -> Optional[str]:
        return (
            ...
            "### Layer 2: User-Driven Customization\n"
            "Every app has two layers:\n"
            "- **Layer 1** — The base app you built. Functional but generic.\n"
            "- **Layer 2** — Deep personalization based on auditing what Layer 1 actually generated.\n\n"
            ...
            "Use `app_{slug}__read_file` to read EVERY key file in the app:\n"
            "- App.tsx, navigation config, every screen component\n"
            "- Database schema files, seed data, constants/config files\n"
            "- Any data files (vocabulary lists, schedules, templates, sample content)\n\n"
            "As you read, build a detailed audit of:\n"
            "- **Placeholder/demo data**: Mock items, generic lists, hardcoded sample entries, lorem-style content\n"
            "- **Shallow features**: Screens that exist but have minimal/static functionality\n"
            "- **Generic defaults**: One-size-fits-all settings, schedules, or goals\n"
            "- **Missing functionality**: Features referenced in UI but not implemented, empty screens\n"
            "- **Hardcoded content**: Text, categories, or values that should be personalized\n\n"
            "Do NOT tell the user you are auditing. Do NOT expose file paths or technical details.\n"
            "Just read the files silently, then proceed to questions.\n\n"
            ...
            "NEVER ask generic onboarding questions (those belong to Layer 1).\n"
            "NEVER repeat questions that Layer 1 already asked (target score, test date, color theme, etc.).\n"
            ...
            "Question patterns based on audit findings:\n"
            "- Found placeholder data → 'I found {N} {items} but they're all generic — should I replace them with {specific alternative}?'\n"
            "- Found generic schedule → 'The study plan uses a default daily routine — want me to restructure it around your actual availability?'\n"
            "- Found shallow feature → 'The {feature} section exists but only has basic {X} — should I add {deeper capability}?'\n"
            "- Found hardcoded content → 'The {section} has {N} sample {items} — should I generate real {domain-specific} content at {level}?'\n"
            "- Found missing capability → 'I notice there's no {X} — should I add {specific implementation}?'\n"
            "- Found simplified logic → 'The {feature} uses a basic formula — want me to implement the proper {domain} calculation?'\n\n"
            "Example for an IELTS prep app (based on actual code audit):\n"
            "```\n"
            "I've analyzed your app's content and structure. Here's what I can improve:\n\n"
            "1. **I found 500 vocabulary words but they're all general English — should I focus them on academic reading passages for your target field?**\n"
            "[[Science & Tech]] [[Business & Economics]] [[Social Sciences]] [[Arts & Humanities]] [[Keep general]]\n\n"
            "2. **The study plan uses a generic 6-task daily schedule — want me to restructure it around your available study hours?**\n"
            "[[30 min/day]] [[1 hour/day]] [[2 hours/day]] [[3+ hours on weekends]] [[Custom schedule]]\n\n"
            "3. **The reading section has 10 passages but they're placeholder text — should I generate real IELTS-style passages at your target difficulty?**\n"
            "[[Band 5-6 level]] [[Band 6-7 level]] [[Band 7-8 level]] [[Mix of difficulties]]\n\n"
            "4. **The mock test scoring uses a simplified formula — want me to implement the official IELTS band descriptor calculation?**\n"
            "[[Yes, use official scoring]] [[Keep it simple]]\n\n"
            "5. **The writing section has generic prompts — should I generate Task 1 & Task 2 prompts focused on your weakest areas?**\n"
            "[[Focus on Task 1 (reports)]] [[Focus on Task 2 (essays)]] [[Both equally]]\n\n"
            "6. **I found the speaking section has no pronunciation feedback — should I add voice analysis for your practice sessions?**\n"
            "[[Yes, add voice feedback]] [[No, text-based is fine]]\n\n"
            "7. **The progress tracker shows overall score but not per-skill trends — want me to add detailed skill breakdown charts?**\n"
            "[[Yes, detailed charts]] [[Keep it simple]]\n\n"
            "8. **The grammar section covers all topics equally — should I prioritize the patterns that lose IELTS candidates the most marks?**\n"
            "[[Yes, focus on high-impact]] [[Cover everything equally]]\n\n"
            "9. **The listening exercises use the same speed — should I add adjustable playback speed for progressive training?**\n"
            "[[Yes, variable speeds]] [[Normal speed only]]\n\n"
            "10. **The app sends no reminders — should I set up a daily study notification at your preferred time?**\n"
            "[[Morning (8 AM)]] [[Afternoon (2 PM)]] [[Evening (7 PM)]] [[No reminders]]\n"
            "```\n\n"
            "Notice: Every question references something SPECIFIC the agent found in the code.\n"
            "The user should feel like the agent deeply understands what the app contains.\n\n"
            ...
            "- Restructure schedules, plans, and workflows to match the user's actual habits\n"
            "- Upgrade shallow features with deeper, functional implementations\n"
            "- Add missing capabilities the user confirmed they want\n"
            "- Adjust difficulty, content density, and pacing based on user level\n\n"
            "Show progress as you make changes. After each file edit, briefly confirm what was improved.\n"
            "The changes are applied live — the user sees updates in real-time.\n\n"
