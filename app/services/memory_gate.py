...
# ── Sensitive values (MOVED) ─────────────────────────────────────────────
#
# The never-store tier now lives in `app/services/memory_secrets.py`. v3's
# writer is the curator, which must apply this rule and must NOT depend on
# the rest of this module — the provenance rules below all measure a
# candidate against the user's own sentence, a measurement v3's source gate
# makes structurally unnecessary. Re-exported here so the row-era callers
# this module still serves keep working unchanged.
from app.services.memory_secrets import (  # noqa: E402
    _CARD_CONTEXT_RE,
    _LUHN_CANDIDATE_RE,
    _NEVER_STORE_LABELS,
    _SENSITIVE_RES,
