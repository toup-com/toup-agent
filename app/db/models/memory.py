...
class MemoryFile(Base):
    """A curated memory file — in v3, THE unit of memory.

    One row per (user, slug). `body_md` is the memory: a markdown bullet
    list the user reads on the file page and the prompt injects verbatim.
    Rows in `memories` are retired from the product (v3 §1.1); nothing here
    counts or joins them any more.

    AGENT_ONLY with no platform mirror. With rows retired the platform has
    nothing to synthesize a virtual view from, so a file read on the
    platform degrades to 503 rather than to a lie (v3 §4).
    """
    ...
    # REQUIRED in v3: "<what this is> — <scope>; read when <trigger>."
    # Generated by the writer, regex-validated, regenerated when the body
    # changes materially. The DB column is still named `purpose` — round 8's
    # name — because renaming a live AGENT_ONLY column fleet-wide buys
    # nothing that an attribute alias does not: every payload, prompt and
    # op says `description`, and no self-heal ALTER is needed.
    description: Mapped[Optional[str]] = mapped_column("purpose", Text, nullable=True)
    # THE BODY. Markdown bullet list; Current context adds `##` layer
    # headings with prose (v3 §6). Capped at MAX_BODY_CHARS by the ops
    # engine, which rejects an op that would overflow it.
    body_md: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    # Cross-links: JSON array of sibling file slugs, validated to exist.
    links_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    # Round-8's name for the same thing. Kept so the column self-heal and
    # WS-5's migration can read what round 8 wrote; v3 writes links_json.
    related_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    # Internal bookkeeping — Current context rollover cursors and the like.
    # NEVER serialized to a client (v3 §1.1); the zero-metadata API test
    # asserts the key never appears in a payload.
    pinned_meta_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    # Order within the section (system files first, then by creation).
    position: Mapped[int] = mapped_column(Integer, default=0)
    is_system: Mapped[bool] = mapped_column(Boolean, default=False)

    # Round-8 curation stamp. Unused by v3 (the curator writes on every
    # turn); retained because the column exists on every live tenant.
...
class MemoryFileChange(Base):
    """The writer's audit trail — one row per applied mutation (v3 §1.1).

    Feeds the Memory log calendar: `GET /api/memories/log?month=YYYY-MM`
    groups these by `day_key`, which is the USER-LOCAL date the change
    happened on (UTC when their zone is unknown). Stored rather than
    derived because `created_at` is UTC and a calendar that buckets a
    23:40-local write into tomorrow is a calendar the user cannot trust.

    `summary` is the human-readable change line the curator was required to
    supply ("Added Music: likes Googoosh."). AGENT_ONLY, no platform mirror.
    """
    __tablename__ = "memory_file_changes"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"), index=True)

    file_slug: Mapped[str] = mapped_column(String(160), index=True)
    # Denormalized on purpose: a log entry has to keep reading correctly
    # after its file is renamed or deleted.
    file_title: Mapped[str] = mapped_column(String(200))
    # created | updated | removed | file_deleted
    ...
    __table_args__ = (
        Index("ix_memory_file_changes_user_day", "user_id", "day_key"),
    )
