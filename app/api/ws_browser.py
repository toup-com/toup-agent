...
async def ws_browser(
    ...
    try:
        ...
        await websocket.send_json({"type": "error", "message": f"Browser not available: {e}"})
    ...
    async def handle_chat(message: str):
        ...
        Uses channel="web" so conversations are shared with the main Chat page.
        The browser context is injected as a system note, keeping the same session.
        """
        ...
        # Stream callback
        async def on_stream(event_type: str, data: dict):
            try:
                if event_type == "text_chunk":
                    await websocket.send_json({"type": "agent_stream", "token": data.get("text", "")})
                elif event_type == "tool_start":
                    await websocket.send_json({"type": "tool_use", "tool": data.get("tool", "")})
        ...
        try:
            ...
            async with async_session_maker() as db:
                result = await _agent_runner.run(
                    ...
                    channel="web",  # Same channel as main Chat — shared conversation
