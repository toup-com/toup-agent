...
async def _chat_complete(
    ...
    #
    # This REST route ran its OWN extraction — `max_memories=5`, per-memory
    # `smart_create_memory`, no `expires_at` from the extractor's ttl and no
    # person names, so a transient fact that survived the horizon check was
    # stored PERMANENT and nothing was person-routed. It was a fourth writer
    # with a fourth idea of the rules.
    #
    # v3 has one writer, `memory_curator.curate_turn`, and it runs from
    # `AgentRunner._background_post_processing` — the WS chat path every
    # client actually uses. This route is a memory-augmented but TOOL-LESS
    # single completion (ws_realtime's own comment says "this MUST NOT be
    # /api/chat"), so it reads memory and writes none. `extracted_memories`
    # stays in the response schema and stays empty.
...
async def _chat_stream(
    ...
    async def _stream_body(db):
        ...
        try:
            ...
            # above: v3's one writer is memory_curator.curate_turn on the
            # agent-runner path, and this legacy REST stream writes none.
            # The `extracted_memories` SSE frame is therefore never emitted.
