...
# ─── Reconciler test — stuck row gets requeued ────────────────────────


@pytest.mark.asyncio
async def test_reconciler_requeues_stuck_provisioning_row(
    test_user_id, agent_config_managed, prewarm_flag_on
):
    """The reconciler tick must detect rows stuck in 'provisioning' for
    longer than _STUCK_PROVISIONING_MIN and re-fire the bridge call.
    This is the recovery path for asyncio.create_task dying mid-flight
    on a Railway redeploy."""
    from app.db import async_session_maker
    from app.db.models.platform import ManagedContainer
    from app.services.prewarm_service import reconcile_stuck_provisioning

    stale_when = datetime.utcnow() - timedelta(minutes=10)

    async with async_session_maker() as db:
        mc = ManagedContainer(
            id=str(uuid.uuid4()),
            user_id=test_user_id,
            container_name=f"toup-agent-{test_user_id[:8]}",
            host_port=9004,
            db_name=f"toup_agent_{test_user_id[:8]}",
            status="provisioning",
            updated_at=stale_when,
        )
        db.add(mc)
        await db.commit()
        # SQLAlchemy's onupdate would bump updated_at on the next mutation,
        # so we set it again via a raw UPDATE to keep it stale for the test.
        from sqlalchemy import text
        await db.execute(
            text(
                "UPDATE managed_containers SET updated_at = :ts WHERE user_id = :uid"
            ),
            {"ts": stale_when, "uid": test_user_id},
        )
        await db.commit()

    requeued_count: list[int] = []
    bridge_called_for: list[str] = []

    async def fake_provision(db, user_id, agent_config=None, recreate=False):
        bridge_called_for.append(user_id)
        return None
    ...
    with patch(
        ...
        n = await reconcile_stuck_provisioning()
        requeued_count.append(n)
        # Yield to let the asyncio.create_task fire
        for _ in range(10):
            await asyncio.sleep(0.05)
            if bridge_called_for:
                break

    assert requeued_count == [1]
    assert bridge_called_for == [test_user_id]
