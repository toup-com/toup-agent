"""Tests for ChatGPT (gpt-image-1) image-generation billing.

Covers:
* image_generation_cost_cents: (size, quality) pricing, the fallback for
  unknown combos, and the dall-e "hd"/"standard"/"auto" quality mappings.
...
def test_image_cost_high_quality_square():
    ...
    assert image_generation_cost_cents("1024x1024", "high") == Decimal("16.7")
...
def test_image_cost_tiers_scale_with_quality():
    ...
    assert low < med < high
    assert (low, med, high) == (Decimal("1.1"), Decimal("4.2"), Decimal("16.7"))


def test_image_cost_landscape_medium():
    from app.services.credit_service import image_generation_cost_cents
    assert image_generation_cost_cents("1536x1024", "medium") == Decimal("6.3")
...
def test_image_cost_default_is_high_quality_square():
    """None/None must resolve to the configured defaults (high, 1024x1024)."""
    from app.services.credit_service import image_generation_cost_cents
    assert image_generation_cost_cents(None, None) == Decimal("16.7")


def test_image_cost_dalle_quality_aliases():
    """dall-e-3 'hd'/'standard' + 'auto' map onto our low/medium/high scale."""
    from app.services.credit_service import image_generation_cost_cents
    assert image_generation_cost_cents("1024x1024", "hd") == Decimal("16.7")       # -> high
    assert image_generation_cost_cents("1024x1024", "standard") == Decimal("4.2")  # -> medium
    assert image_generation_cost_cents("1024x1024", "auto") == Decimal("16.7")     # -> high
...
async def test_image_charge_deducts_and_stamps_event_type(credit_user, monkeypatch):
    """A medium-quality image (4.2cr, under the free daily cap) deducts cleanly
    and stamps event_type='image_generation' on the ledger row."""
    from app.config import settings
    from app.db import async_session_maker
    ...
    cents = image_generation_cost_cents("1024x1024", "medium")  # 4.2
    credits = underlying_cost_to_credits(cents)                 # 4.2
...
async def test_high_quality_image_exceeds_free_daily_cap_when_enforced(credit_user, monkeypatch):
    """DOCUMENTS a real interaction: a HIGH-quality image (16.7cr) exceeds the
    free-tier 15-credit daily message cap, so with enforcement ON a fresh free
    user is denied with daily_cap_exceeded and nothing is deducted.
    ...
    async with async_session_maker() as db:
        result = await credit_service.try_charge(
            db, credit_user, LEDGER_IMAGE_GEN, BUCKET_MESSAGE, Decimal("16.7"),
